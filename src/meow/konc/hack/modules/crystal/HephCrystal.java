...
@Module.Info(name = "HephCrystal", category = Module.Category.CRYSTAL)
public class HephCrystal extends Module {
    ...
    private void drawBlock(BlockPos blockPos, int r, int g, int b) {
        ...
        if (renderMode.getValue().equals(RenderMode.UP)) {
            KONCTessellator.drawBox(blockPos, color.getRGB(), GeometryMasks.Quad.UP);
        } else if (renderMode.getValue().equals(RenderMode.BLOCK)) {
            KONCTessellator.drawBox(blockPos, color.getRGB(), GeometryMasks.Quad.ALL);
        }
    ...
    public void onEnable() {
        if (announceUsage.getValue()) {
            Command.sendChatMessage("[HephCrystal] " + ChatFormatting.GREEN.toString() + "Enabled!");
        }
        hitDelayCounter = 0;
    }
    ...
    public void onDisable() {
        ...
        resetRotation();
        if (announceUsage.getValue()) {
            Command.sendChatMessage("[HephCrystal] " + ChatFormatting.RED.toString() + "Disabled!");
        }
    }
