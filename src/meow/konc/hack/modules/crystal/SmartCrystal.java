...
@Module.Info(name = "SmartCrystal", category = Module.Category.CRYSTAL)
public class SmartCrystal extends Module {
    public static Entity renderEnt;
    public static boolean togglePitch = false;
    public static boolean isSpoofingAngles;
    public static float yaw;
    public static float pitch;
    public Setting<typesetting> typeset = register(Settings.enumBuilder(typesetting.class).withName("Type").withValue(typesetting.GENERAL));
            ...
            if (renderEnt != null) {
                for (Entity kami : entityrefresh) {
                    ...
                    for (EntityEnderCrystal crystal2 : lyowo) {
                        if (mc.player.getDistance(crystal2) > range.getValue() || (!mc.player.canEntityBeSeen(crystal2) && mc.player.getDistance(crystal2) > this.walls.getValue() && this.wall.getValue())) {
                            break;
                        }
                        ...
                        if (owokamiisgud.getValue().equals(owokami.MostDamage) ? enemydmgcal > enemydmg : enemydmgcal - owo > enemydmgdiff) {
                            if ((owo + this.antideath.getValue() > mc.player.getHealth() + mc.player.getAbsorptionAmount() && this.suicideprot.getValue()) || (owo > this.maxhitdmg.getValue() && maxselfcalculations.getValue())) {
                                continue;
                            }
    ...
    public void onWorldRender(RenderEvent event) {
        if (this.render != null) {
            ...
            if (rainbow.getValue()) {
                ...
                KONCTessellator.drawBox(this.render, red2, green2, blue2, this.alpha.getValue(), 63);
            } else {
                KONCTessellator.prepare(7);
                KONCTessellator.drawBox(this.render, this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue(), 63);
            }
            ...
            if (box.getValue()) {
                ...
                if (rainbow2.getValue()) {
                    ...
                    KONCTessellator.drawBoundingBox(iblockstate.getSelectedBoundingBox((World) mc.world, render).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z), width.getValue(), this.red.getValue(), this.green.getValue(), this.blue.getValue(), 255);
                }
            }
            ...
            if (renderdamage.getValue()) {
                ...
                if (accuratedamagerender.getValue()) {
                    ...
                    mc.fontRenderer.drawStringWithShadow("\u00a7b" + damage, 0, 10, 0xFFAAAAAA);
                    ...
                    GlStateManager.disableDepth();
                    GlStateManager.translate(-(mc.fontRenderer.getStringWidth(damageText) / 2.0d), 0, 0);
                    mc.fontRenderer.drawStringWithShadow("\u00a7b" + damageText, 0, 10, 0xFFAAAAAA);
                    GlStateManager.popMatrix();
                }
            }
        }
    ...
    public List<BlockPos> findCrystalBlocks() {
        ...
        positions.addAll(this.getSphere(new BlockPos(getPlayerPos().x, getPlayerPos().y + 1, getPlayerPos().z), this.placerange.getValue().floatValue(), this.placerange.getValue().intValue(), false, true, 0).stream().filter(this::canPlaceCrystal).collect(Collectors.toList()));
        return positions;
    }
