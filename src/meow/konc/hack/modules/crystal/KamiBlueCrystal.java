...
import meow.konc.hack.util.other.GeometryMasks;
...
import static meow.konc.hack.util.colour.ColourConverter.rgbToInt;
import static meow.konc.hack.util.colour.ColourConverter.toF;
import static meow.konc.hack.util.util.EntityUtil.calculateLookAt;

@Module.Info(name = "KamiBlueCrystal", category = Module.Category.CRYSTAL, description = "Places End Crystals to kill enemies")
public class KamiBlueCrystal extends Module {
        ...
        if (place.getValue() && placeBehavior.getValue() == PlaceBehavior.MULTI) {
            for (Entity entity : entities) {
                ...
                for (BlockPos blockPos : blocks) {
                    ...
                    double d = calculateDamage(blockPos.x + .5, blockPos.y + 1, blockPos.z + .5, entity);
                    double self = calculateDamage(blockPos.x + .5, blockPos.y + 1, blockPos.z + .5, mc.player);
                    if (self >= mc.player.getHealth() + mc.player.getAbsorptionAmount() || self > d) {
                        continue;
                    }
    ...
    private boolean passSwordCheck() {
        if (!noToolExplode.getValue() || antiWeakness.getValue()) {
            return true;
        } else {
    ...
    private boolean ignored(Entity e) {
        if (ignoredCrystals == null) {
            return false;
        }
        for (EntityEnderCrystal c : ignoredCrystals) {
            if (e != null && e == c) {
                return true;
            }
        }
